...
export abstract class Tests {
  ...
  protected getTestCommandWithFilePattern(): string {
    let command: string = (vscode.workspace.getConfiguration('qcumberExplorer', null).get('rspecCommand') as string);
    ...
    childProcess.exec(cmd, execArgs, (err, stdout) => {
      if (err) {
        ...
        vscode.window.showWarningMessage(
          "QCumber Test Explorer failed to find a .quke test suite. Make sure RSpec is installed and your configured RSpec command is correct.",
          "View error message"
        ).then(selection => {
  ...
  */
  ...
  getTestDirectory(): string {
    let directory: string = (vscode.workspace.getConfiguration('qcumberExplorer', null).get('rspecDirectory') as string);
